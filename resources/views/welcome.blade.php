...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Mero Sewa Admin Dashboard</title>
        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/worker') }}" class="ml-4 css-button-rounded">Home</a>
                    @else
                        <a href="{{ route('login') }}" class="ml-4 css-button-rounded">Log in</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="ml-4 css-button-rounded">Register</a>
                        @endif
                    @endauth
                </div>
            @endif
                <div class="container">
                    <div class="col-sm-6">
                        <h1 style="color: #00BDFE; font-size:90px;">Mero Sewa</h1>
